...
#define HX_PROC_TOUCHSCREEN_FOLDER "touchscreen"
#define HX_PROC_CTP_OPENSHORT_TEST_FILE "ctp_openshort_test"
...
struct proc_dir_entry *hx_proc_touchscreen_dir;
EXPORT_SYMBOL(hx_proc_touchscreen_dir);

struct proc_dir_entry *hx_proc_ctp_openshort_test_file;
EXPORT_SYMBOL(hx_proc_ctp_openshort_test_file);
...
static int wt_self_test_seq_read(struct seq_file *s, void *v)
{
	size_t ret = 0;
...
static const struct seq_operations wt_self_test_seq_ops = {
	.start	= wt_fac_self_test_seq_start,
	.next	= wt_fac_self_test_seq_next,
	.stop	= wt_fac_self_test_seq_stop,
	.show	= wt_self_test_seq_read,
};

static int wt_openshort_test_open(struct inode *inode, struct file *file)
{
    return seq_open(file, &wt_self_test_seq_ops);
}
static ssize_t wt_fac_self_test_write(struct file *filp, const char __user *buff,	size_t len, loff_t *data)
{	
	return len;
}

static const struct file_operations  hx_proc_ctp_openshort_test_fops= {
	.owner = THIS_MODULE,
	.open = wt_openshort_test_open,
	.read = seq_read,
...
{
	hx_proc_touchscreen_dir = proc_mkdir(HX_PROC_TOUCHSCREEN_FOLDER , NULL);
   	if (hx_proc_touchscreen_dir == NULL)  {
		printk(KERN_ERR "%s: proc_touchpanel_dir file create failed!\n", __func__);
	}else{
        hx_proc_ctp_openshort_test_file = proc_create(HX_PROC_CTP_OPENSHORT_TEST_FILE,
		(S_IWUSR | S_IRUGO), hx_proc_touchscreen_dir, &hx_proc_ctp_openshort_test_fops);

		if(hx_proc_ctp_openshort_test_file ==NULL){
			printk(KERN_ERR "%s: proc_ctp_openshort_test_file create failed!\n", __func__);
			remove_proc_entry(HX_PROC_TOUCHSCREEN_FOLDER, NULL);
		}else{
...
static void himax_vsn_reset_device(void)
{
	I("lcm_bais_enn_enable start\n");
	lcm_bais_enn_enable(0);
	I("lcm_bais_enn_enable end\n");
}

static void himax_mcu_vsn_recovery(void)
{
	himax_vsn_reset_device();
}
 ...
 */
	...
	if ((hx_excp_event == length || hx_zero_event == length)
		...
		if (private_ts->suspended == true) {
			if (shaking_ret == HX_EXCP_ZERO_EVENT ) {
#if defined(DD_VSN_RECOV_EXCP)	
			private_ts->gesture_fail_flag = true;
			I("%s: gesture_fail_flag = %d\n", __func__,private_ts->gesture_fail_flag);
#endif	
			}
		} else {
			if (shaking_ret == HX_EXCP_EVENT || 
...
#if defined(DD_VSN_RECOV_EXCP)
			if ( private_ts->gesture_fail_flag  == true ) {
				if (private_ts->update_fw_fail == 1) 
				{
					private_ts->gesture_fail_flag = false;
					private_ts->update_fw_fail = 0;
					private_ts->vsn_flag = 1;
					himax_mcu_vsn_recovery();//��VSN
					I("%s: recovery by unnormal gesture\n", __func__ );
				}				
			}else {
				if (private_ts->update_fw_fail == 2) {
					private_ts->update_fw_fail = 0;
					private_ts->vsn_flag = 1;
					himax_mcu_vsn_recovery();//vsn low
					I("%s: recovery normal\n", __func__ );
				}
			}
...
#if defined(HX_BOOT_UPGRADE) || defined(HX_ZERO_FLASH)
	for(upgrade_num = 0;upgrade_num < 3; upgrade_num++){
		fw_sts = i_get_FW();
		if(fw_sts < NO_ERR)
			msleep(1000);
		else
