...
#define MP_TEST_LC_INI   "/vendor/firmware/gcore_mp_LC_test.ini"
...
int gcore_mp_test_item_noise(struct gcore_mp_data *mp_data)
{
	...
	if (!wait_for_completion_interruptible_timeout(&mp_test_complete, 2 * HZ)) {
...
static int firstline = 1; //20220324add02
			       ...
			       int cols, const char *prefix, const char *suffix, char seperator)
{
	...
	for (c = 0; c < cols; c++) {	//20220324add02
		if(!firstline){
...
int gcore_save_mp_data_to_file(struct gcore_mp_data *mp_data)
{
	...
	if ((dev_mkdir(MP_TEST_DATA_DIR , S_IRWXU | S_IRWXG | S_IRWXO)) != 0)
		GTP_ERROR("Failed to create directory for mp_test\n");
...
#define MP_BIN_LC_NAME "GC7202_mp_LC.bin"
...
extern int gcore_set_pllclk_tp_sel(void); //20220324add
...
#if MP_SET_PLLCLK_TP
		gcore_set_pllclk_tp_sel(); //20220324add

		usleep_range(1000, 1100); //20220324add
#endif
